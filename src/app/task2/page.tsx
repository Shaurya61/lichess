  ...
  const renderKeyValue = (key: string, value: string) => {
    return (
      <div key={key} className="m-2">
        {key} : {" "}
        {typeof value === "object" ? renderPreferences(value as any) : value}
      </div>
    );
  };
    ...
    <div className="w-full">
      <div className="grid justify-items-center rounded-full">
        <div className="text-center flex-col bg-violet-200 rounded-lg w-3/4 pb-8">
          ...
          <h1 className="font-medium text-3xl m-4">
          ...
          <Link href={{ pathname: "/task3", query: { username: username } }} prefetch={false}>
            <div>
              <Button
                className="rounded-full m-8 p-8 text-xl"
                onClick={() => router.push(`/task3?username=${username}`)}
              >
                Move to Task 3
              </Button>
            </div>
      ...
      <div>
        <h1 className="font-bold text-2xl m-8 p-8 bg-purple-100 items-center rounded-lg text-center ml-96 mr-96">
          User Information
        </h1>
